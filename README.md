...
## 注意事项 / Notes
- 为了获得最佳体验，请确保加载正确格式的JSON文件 / For best experience, ensure correct format of JSON files
- 推荐功能需要有效的节点和边数据文件才能正常工作 / Recommendation function requires valid nodes and edges data files
- 在file://协议下，浏览器出于安全考虑会限制自动加载本地文件 / Under file:// protocol, browsers restrict automatic loading of local files for security reasons